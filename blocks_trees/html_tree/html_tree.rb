...
class HTML_Tree
  ...
  def HTML_Tree.is_tag_closing?(open_tag, close_tag)
    return (open_tag != close_tag) && (open_tag == close_tag.sub('/', ''))
  end

  def add_node(node, hash_tags, i)
    if i < hash_tags.size
      if HTML_Tree.is_tag_closing?(self.tag_stack.last, hash_tags[i][:name]) == true
        self.tag_stack.pop
        ...
        new_node = Tag.new(name: hash_tag[:name], features: hash_tag[:features], info: hash_tag[:info], parent: node)
